# SPEC: Terminal Non-Blocking Hang-Resilience -- v2.0

Status: Implemented & Shipped (feature/rw-live @ 758ff6de41) -- v2 incorporates triage fixes (spill path, drain join, concurrent hangs, checkpoint recovery)
Date: 2026-08-31
...
Forward audit: 17/18 pass; multiplex code-gated fallback required (F18).

Reverse gaps fixed:
- R3: Promoted payload now explicitly includes `full_output_path` when `bounded_capture` spill overflowed, else omitted. Agent must read spill file for tail.
- Drain join: `drain_thread.join(timeout=2)` is part of promotion contract (spec 3 updated).
- Concurrent hangs: `_idle_last_output_at` and `_promotion_event` are per-wait locals (list/event), not globals -- add invariant.
- Checkpoint: adopted session persists via `_write_checkpoint`; add AC: `poll` after simulated gateway restart still returns running.
- Sandbox error shape: canonical `{multiplex_unsupported:true, fallback:"popen"}` for multiplex; promotion never applies there.

Bug review B3: Adopted session carries `spill_path` (new ProcessSession field) so poll tail survives adopt.

Lint: phase0-recon/*.log to .gitignore; remove backup branch.

Test/Perf: formal pytest for 3 edges + silent_for_seconds added to polish gate.
